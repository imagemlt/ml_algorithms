...
booleanPersonGenerator::booleanPersonGenerator(){       //this->path=path;
        this->target=vector<double>(2);
        this->input=vector<double>(961);
}

int booleanPersonGenerator::getOutputSize(){
    return 2;
}

vector<double> booleanPersonGenerator::getInput(string path,string filename){
        string realfile;
        string subname;
...
vector<double> booleanPersonGenerator::getTarget(string path,string filename){
    int dire;
            //cout<<line<<line.find("_straight_")<<" "<<line.find("_right_")<<" "<<line.find("_left_")<<" "<<line.find("_up_")<<endl;
    if(filename.find("an2i")!=string::npos)dire=0;
    else dire=1;
    //cout<<filename<<"\t"<<dire<<endl;
