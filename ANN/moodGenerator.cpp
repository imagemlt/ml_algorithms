...
moodGenerator::moodGenerator(){       //this->path=path;
        this->target=vector<double>(4);
        this->input=vector<double>(961);
        //this->faces=;
}

int moodGenerator::getOutputSize(){
    return 4;
}

vector<double> moodGenerator::getInput(string path,string filename){
        string realfile;
        string subname;
...
vector<double> moodGenerator::getTarget(string path,string filename){
    int dire=0;
    ...
    //else dire=1;
    for(int m=0;m<4;m++){
        if(filename.find(moods[m])!=string::npos){
            dire=m;
            break;
        }
    }
